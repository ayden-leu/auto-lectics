...
signal option_changed_history(oldFlagID:String, newFlagID:String)
...
## Usage:
## [codeblock]
## var flagField:DC_StoryFlagFieldOption = # a pre-configured node from the scene tree
##
...
## [b]Internal-use only.[/b]  Runs when either [DC_AspectsCheckFlags] or [DC_AspectsSetFlags]
...
func _on_chooser_updated(_index:int) -> void:
	option_changed_history.emit(_prevSelectedFlagID, flagID)
